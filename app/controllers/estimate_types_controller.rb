class EstimateTypesController < ApplicationController
  ...
  def destroy
    respond_to do |format|
      if @estimate_type.destroy
        format.html { redirect_to estimate_types_url, notice: 'Estimate Type was successfully deleted.' }
        format.json { head :no_content }
      else
        format.html { redirect_to estimate_types_url, notice: "An Error Occurred! #{@estimate_type.errors[:base].to_s}"}
        format.json { head :ok }
      end
    end
  end
