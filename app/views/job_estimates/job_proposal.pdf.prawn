...
if (@job_estimate_items.size > 0)
    ...
    @job_estimate_items.each do |job_estimate_item|
        pdf.draw_text job_estimate_item.step, :at => [12, y_axis], :size => 11
        pdf.draw_text Assembly.where(:id=>job_estimate_item.master_item_id).pluck(:item_description).first, :at => [57, y_axis], :size => 11
        y_axis = y_axis - 30
    end
end
